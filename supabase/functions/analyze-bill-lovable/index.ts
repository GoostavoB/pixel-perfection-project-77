...
serve(async (req) => {
  ...
  try {
    ...
    const providedSessionId = formData.get('sessionId') as string | null;
    const sessionId = providedSessionId || crypto.randomUUID();
    console.log('Session ID:', sessionId, providedSessionId ? '(provided)' : '(generated)');
...
`;
  ...
  // ✅ EMBEDDED PROMPT: Avoid filesystem dependency
  const systemPrompt = `# Medical Bill Data Extraction Prompt
...
5. No extra analysis fields are added`;
  ...
  console.log('[PROMPT] Using embedded data extraction prompt, length:', systemPrompt.length);
